...
async def test_ssl_check_expired_cert(mock_socket, mock_ssl):
    ...
    result = await run("example.com")
    assert result.valid == True # Cert format parsed ok
    assert result.days_until_expiry < 0
...
async def test_cookie_check_detects_missing_httponly(mock_client):
    ...
    mock_instance = mock_client.return_value.__aenter__.return_value
    mock_response = MagicMock()
    mock_response.headers.get_list.return_value = ["session_id=123; Secure; SameSite=Lax"] # Missing HttpOnly
    mock_instance.get.return_value = mock_response
    
    result = await run("https://example.com")
    assert result.total_cookies == 1
    assert any(c['name'] == 'session_id' and 'HttpOnly' in c['missing_flags'] for c in result.insecure_cookies)
    assert result.session_cookies_insecure == True
