...
def test_valid_urls():
    assert SSRFValidator.validate_url("https://www.google.com") == "https://www.google.com"
    assert SSRFValidator.validate_url("http://example.com/path?query=1") == "http://example.com/path?query=1"
...
def test_blocked_ips():
    ...
    for url in bad_ips:
        with pytest.raises(SSRFValidationError, match="Blocked internal/reserved IP address"):
