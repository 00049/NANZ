...
class ReportShareLink(Base):
    ...
    __table_args__ = (Index("idx_report_shares_scan", "scan_id", "is_revoked"),)
...
class ReportAuditLog(Base):
    ...
    share_token_used = Column(
        String(255), nullable=True
    )  # If accessed via a share link

    timestamp = Column(
        DateTime(timezone=True), server_default=text("now()"), index=True
    )

    __table_args__ = (Index("idx_report_audit_scan", "scan_id", "timestamp"),)
