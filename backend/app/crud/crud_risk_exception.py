...
def get_exception(db: Session, exception_id: UUID) -> RiskException | None:
    return db.query(RiskException).filter(RiskException.id == exception_id).first()


def get_active_exceptions_for_domain(
    ...
    return (
        ...
        .filter(
            ...
            (RiskException.expires_at is None) | (RiskException.expires_at > now),
        )
        .all()
    )


def get_exception_by_key(
    db: Session, domain_id: UUID, finding_key: str
) -> RiskException | None:
    now = datetime.now(UTC)
    return (
        db.query(RiskException)
...
def create_exception(
    db: Session, domain_id: UUID, user_id: UUID, exception_in: RiskExceptionCreate
) -> RiskException:
    ...
    if existing:
        return update_exception(
            db, existing, RiskExceptionUpdate(**exception_in.model_dump()), user_id
        )
...
def update_exception(
    db: Session, db_obj: RiskException, exception_in: RiskExceptionUpdate, user_id: UUID
) -> RiskException:
...
def remove_exception(db: Session, db_obj: RiskException, user_id: UUID) -> None:
    ...
    db_obj.expires_at = datetime.now(UTC)
    db.add(db_obj)
