...
def compute_soc2_report(
    ...
    if hits:
        ...
        monitoring_gaps.append(
            "Error verbosity in production responses — disable debug mode"
        )
        monitoring_gaps.append("No rate limiting detected on sensitive endpoints")

    if "llm_no_rate_limiting" in finding_keys:
        monitoring_gaps.append(
            "LLM endpoints lack rate limiting — unbounded consumption not monitored"
        )
    ...
    status = (
        "Compliant" if score >= 80 else ("Partial" if score >= 50 else "Non-Compliant")
    )
    ...
    if "iast_db_error" not in all_keys and "iast_stack_trace" not in all_keys:
        passing_controls.append(
            "CC7.2 — No error verbosity or stack trace leakage detected"
        )
