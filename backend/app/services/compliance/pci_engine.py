    ...
    def to_dict(self) -> dict:
        return {
            ...
            "violated_requirements": [
                _req_to_dict(r) for r in self.violated_requirements
            ],
...
def compute_pci_report(
    ...
    finding_keys = {
        f.get("key", "") or f.get("check_type", "") or "" for f in classified_findings
    }
    ...
    if hits:
        ...
        audit_evidence.append(
            {"requirement": "Req 4.2.1", "violations": list(hits), "timestamp": now_ts}
        )
    ...
    if hits:
        ...
        audit_evidence.append(
            {"requirement": "Req 6.3.3", "violations": list(hits), "timestamp": now_ts}
        )
    ...
    if hits or waf_absent:
        ...
        violations.append(
            PCIRequirementViolation(
                ...
                description=(
                    ...
                    + (
                        "No WAF detected — automated protection is missing. "
                        if waf_absent
                        else ""
                    )
                    + (
                        "Known web application vulnerabilities were detected. "
                        if hits
                        else ""
                    )
                ),
        ...
        audit_evidence.append(
            {"requirement": "Req 6.4.1", "violations": all_hits, "timestamp": now_ts}
        )
    ...
    if hits:
        ...
        audit_evidence.append(
            {"requirement": "Req 7.2", "violations": list(hits), "timestamp": now_ts}
        )
    ...
    if hits:
        ...
        audit_evidence.append(
            {"requirement": "Req 1.3.2", "violations": list(hits), "timestamp": now_ts}
        )
