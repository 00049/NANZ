...
class DPDPReport:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "violated_sections": [
                _violation_to_dict(v) for v in self.violated_sections
            ],
...
def compute_dpdp_report(
    ...
    if "ssl_invalid" not in all_keys and "ssl_unavailable" not in all_keys:
        passing_controls.append(
            "Valid SSL/TLS certificate — data encrypted in transit (S.8(4))"
        )
    if "domain_in_breach" not in all_keys:
        passing_controls.append(
            "No known data breaches detected — breach notification (S.8(6)) not triggered"
        )
    if "ports_database_exposed" not in all_keys:
        passing_controls.append(
            "Database ports not publicly exposed — access controls in place (S.8(4))"
        )
    if "headers_no_https_redirect" not in all_keys:
        passing_controls.append(
            "HTTP forced to HTTPS — encrypted data transmission enforced (S.8(4))"
        )
    ...
    audit_log = {
        ...
        "trigger_evidence": {v.section: v.trigger_findings for v in violations},
    }
