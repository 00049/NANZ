...
class CVEFinding:
    ...
    kev_entry: Optional[dict] = None   # Full KEV metadata if available
...
async def _enrich_cves_with_threat_intel(cves: List[CVEFinding]) -> None:
    ...
    try:
        ...
        logger.debug("Threat intel clients not available — skipping EPSS/KEV enrichment")
    except Exception as e:
        ...
        async def _bounded_query(tech: Dict[str, str]):
            async with sem:
                ...
                for c in cves:
                    ...
                    elif c.epss_score and c.epss_score > 0.5 and highest_sev not in ("CRITICAL",):
                        highest_sev = "HIGH"
