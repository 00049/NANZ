...
STRONG_WAFS = {"Cloudflare", "AWS CloudFront", "Akamai", "Imperva", "Sucuri", "Azure Front Door"}

SEVERITY_DOWNGRADE = {"CRITICAL": "RED", "RED": "AMBER", "AMBER": "GREEN", "GREEN": "INFO"}


def classify_findings(raw: dict, waf_context: Optional[dict] = None) -> list[dict]:
    findings: list[dict] = []
...
def _classify_brand_threats(brand: dict, findings: list) -> None:
    ...
    for t in threats:
        ...
        severity_map = {"CRITICAL": "CRITICAL", "HIGH": "RED", "MEDIUM": "AMBER", "LOW": "GREEN"}
        severity = severity_map.get(threat_level, "AMBER")

        label = {"typosquat": "Typosquatting Domain", "homoglyph": "Homoglyph/Impersonation Domain", "ct_alert": "CT Log Brand Alert"}
        key   = {"typosquat": "brand_typosquat", "homoglyph": "brand_homoglyph", "ct_alert": "brand_ct_alert"}

        _add(findings, "brand", severity, key.get(threat_type, "brand_threat"), t,
...
def _classify_ssl(ssl: dict, findings: list) -> None:
    ...
    if isinstance(days, (int, float)) and days < 14:
        _add(findings, "ssl", "RED", "ssl_expiring_critical", ssl, detail=f"Certificate expires in {int(days)} days")
    elif isinstance(days, (int, float)) and days <= 30:
        _add(findings, "ssl", "AMBER", "ssl_expiring_soon", ssl, detail=f"Certificate expires in {int(days)} days")
...
def _classify_headers(headers: dict, findings: list) -> None:
    ...
    if sensitive_robots:
        _add(findings, "headers", "AMBER", "headers_robots_sensitive_paths", headers,
...
def _classify_ports(ports: dict, findings: list) -> None:
    ...
    if exposed_db:
        port_names = {3306: "MySQL", 5432: "PostgreSQL", 27017: "MongoDB", 6379: "Redis", 9200: "Elasticsearch"}
...
def _classify_webapp(webapp: dict, findings: list) -> None:
    ...
    for ef in exposed_files:
        ...
        _add(findings, "webapp", severity, key, webapp,
...
def _classify_javascript(js: dict, findings: list) -> None:
    ...
    for secret in js.get("secrets_found", []):
        ...
        if pt in ("aws_access_key", "private_key", "stripe_key"):
            _add(findings, "javascript", "CRITICAL", "aws_key_in_source" if "aws" in pt else "api_key_in_source", js,
                 detail=f"{pt.replace('_', ' ').title()} Pattern Detected in Client-Side Code")
        else:
            _add(findings, "javascript", "RED", "api_key_in_source", js,
                 detail=f"{pt.replace('_', ' ').title()} Pattern Detected in Client-Side Code")
    for sm in js.get("source_maps_exposed", []):
        _add(findings, "javascript", "RED", "source_map_exposed", js,
             detail=f"JavaScript Source Map Publicly Accessible")
...
def _classify_http_methods(methods: dict, findings: list) -> None:
    ...
    if methods.get("trace_reflected"):
        _add(findings, "http_methods", "CRITICAL", "trace_with_reflection", methods,
             detail="TRACE Method Enabled with Header Reflection (Cross-Site Tracing)")
    elif methods.get("trace_enabled"):
        _add(findings, "http_methods", "RED", "trace_enabled", methods,
    ...
    if non_trace:
        _add(findings, "http_methods", "AMBER", "dangerous_methods_enabled", methods,
...
def _classify_cloud(cloud: dict, findings: list) -> None:
    ...
    for bucket in cloud.get("public_buckets", []):
        ...
        _add(findings, "cloud", "CRITICAL", "public_cloud_bucket", cloud,
             detail=f"Publicly Listable {provider} Bucket: {name}")
    for bucket in cloud.get("protected_buckets", []):
        name = bucket.get("name", "unknown")
        _add(findings, "cloud", "AMBER", "enumerable_cloud_bucket", cloud,
