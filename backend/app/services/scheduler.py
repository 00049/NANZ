...
async def execute_scheduled_scan(
    schedule: ScanSchedule, db: AsyncSession, redis_client
) -> None:
    ...
    result = await create_new_scan(
        schedule.domain, schedule.domain, "127.0.0.1", db, redis_client
    )
    ...
    if "error" not in result:
        ...
        logger.error(
            f"Failed to execute scheduled scan for {schedule.domain}: {result['error']}"
        )
