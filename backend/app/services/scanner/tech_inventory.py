...
async def _extract_tech_from_headers(
    headers: httpx.Headers, result: TechInventoryResult
):
    ...
    if server:
        ...
        result.technologies.append(
            {"name": name.capitalize(), "type": "Web Server", "version": version}
        )
    ...
    if x_powered_by:
        ...
        result.technologies.append(
            {"name": name, "type": "Backend Framework/Language", "version": version}
        )
...
async def _analyze_html(html: str, result: TechInventoryResult):
    ...
    for meta in soup.find_all("meta", attrs={"name": "generator"}):
        ...
        if content:
            ...
            result.technologies.append(
                {"name": name, "type": "CMS", "version": version}
            )
    ...
    for script in scripts:
        if "jquery" in script.lower():
            ...
            result.technologies.append(
                {"name": "jQuery", "type": "JavaScript Library", "version": version}
            )
        elif "bootstrap" in script.lower():
            match = re.search(
                r"bootstrap(?:[-.]min)?(?:[-.]js)?@(\d+\.\d+\.\d+)", script.lower()
            )
            if not match:
                match = re.search(r"bootstrap/(\d+\.\d+\.\d+)", script.lower())
            version = match.group(1) if match else ""
            result.technologies.append(
                {"name": "Bootstrap", "type": "UI Framework", "version": version}
            )
        elif "react" in script.lower():
            result.technologies.append(
                {"name": "React", "type": "JavaScript Framework", "version": ""}
            )
        elif "vue" in script.lower():
            result.technologies.append(
                {"name": "Vue.js", "type": "JavaScript Framework", "version": ""}
            )
