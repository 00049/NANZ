...
STACK_TRACE_PATTERNS = [
    ...
    (
        re.compile(
            r"RuntimeError|AttributeError|KeyError|ValueError.*File.*line \d+",
            re.IGNORECASE,
        ),
        "Python error detail",
    ),
    (
        re.compile(r"System\..*Exception|StackTrace|at System\.", re.IGNORECASE),
        ".NET stack trace",
    ),
    (
        re.compile(
            r"Parse error:|Fatal error:|Warning:|Notice:\s+.*in\s+/", re.IGNORECASE
        ),
        "PHP error",
    ),
    (
        re.compile(r"ActionController::|ActionDispatch::|NoMethodError", re.IGNORECASE),
        "Ruby on Rails error",
    ),
]
...
DB_ERROR_PATTERNS = [
    ...
    (
        re.compile(r"mysql_connect|mysqli_connect|pg_query|pg_connect", re.IGNORECASE),
        "DB connection function",
    ),
    (re.compile(r"ORA-\d{5}", re.IGNORECASE), "Oracle DB error"),
    (
        re.compile(r"syntax error.*near|near.*syntax error", re.IGNORECASE),
        "SQL syntax error",
    ),
    (
        re.compile(r"relation .* does not exist|table .* doesn't exist", re.IGNORECASE),
        "DB schema hint",
    ),
    (
        re.compile(r"column .* of relation|unknown column .* in", re.IGNORECASE),
        "DB column name",
    ),
    (re.compile(r"Microsoft.*SQL Server|OLE DB.*SQL", re.IGNORECASE), "MSSQL error"),
    (
        re.compile(r"psycopg2\.errors|asyncpg\.exceptions", re.IGNORECASE),
        "PostgreSQL Python error",
    ),
]
...
class IASTBehavioralResult:
    stack_traces_found: list = field(
        default_factory=list
    )  # {url, framework, pattern, severity}
    ...
    timing_anomalies: list = field(
        default_factory=list
    )  # {probe_type, baseline_ms, probe_ms, ratio}
...
async def _run_error_probes(
    client: httpx.AsyncClient, base_url: str, result: IASTBehavioralResult
) -> None:
    ...
    probes = [
        ...
        # Unexpected HTTP method
        {
            "url": base_url,
            "method": "PATCH",
            "desc": "unexpected_method",
            "extra_headers": {},
        },
        # Numeric parameter edge cases
        {
            "url": urljoin(base, "/?id=0"),
            "method": "GET",
            "desc": "id_zero",
            "extra_headers": {},
        },
        {
            "url": urljoin(base, "/?id=-1"),
            "method": "GET",
            "desc": "id_negative",
            "extra_headers": {},
        },
...
async def _run_timing_analysis(
    client: httpx.AsyncClient, url: str, result: IASTBehavioralResult
) -> None:
...
async def _timed_get(client: httpx.AsyncClient, url: str) -> float | None:
