    ...
    if key in _CATALOG:
        ...
        return {
            ...
            "observation": content.get(
                "observation", "A security misconfiguration was detected."
            ),
            "impact": content.get(
                "impact",
                "Could be leveraged by an attacker to compromise application integrity or data confidentiality.",
            ),
            "remediation": content.get(
                "remediation",
                "Review the finding details and apply security best practices to resolve the issue.",
            ),
            "verification": content.get(
                "verification",
                "Retest the endpoint after applying the fix to ensure the vulnerability is resolved.",
            ),
        }
