...
FIX_KNOWLEDGE: dict[str, dict] = {
    ...
    "ssl": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Audit current TLS configuration",
                "description": "Check which TLS versions and cipher suites your server currently supports.",
                "code_snippet": "nmap --script ssl-enum-ciphers -p 443 yourdomain.com",
                "code_language": "bash",
            },
            {
                "order": 2,
                "title": "Disable legacy TLS versions",
                "description": "Update your web server configuration to only allow TLS 1.2 and TLS 1.3. Remove support for TLS 1.0 and TLS 1.1.",
                "code_snippet": "# Nginx: /etc/nginx/nginx.conf\nssl_protocols TLSv1.2 TLSv1.3;\nssl_ciphers 'TLS_AES_128_GCM_SHA256:TLS_AES_256_GCM_SHA384:ECDHE-RSA-AES256-GCM-SHA384';\nssl_prefer_server_ciphers off;",
                "code_language": "nginx",
            },
            {
                "order": 3,
                "title": "Enable HSTS header",
                "description": "Add the Strict-Transport-Security header to force browsers to always use HTTPS.",
                "code_snippet": 'add_header Strict-Transport-Security "max-age=31536000; includeSubDomains; preload" always;',
                "code_language": "nginx",
            },
            {
                "order": 4,
                "title": "Verify the fix",
                "description": "Test your SSL configuration using an online scanner or command line tool.",
                "code_snippet": "curl -sI https://yourdomain.com | grep -i strict",
                "code_language": "bash",
            },
        ],
    ...
    "headers": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Add security headers to your server",
                "description": "Configure your web server or application to include all recommended security headers in every HTTP response.",
                "code_snippet": '# Nginx — add to server block\nadd_header X-Content-Type-Options "nosniff" always;\nadd_header X-Frame-Options "DENY" always;\nadd_header X-XSS-Protection "1; mode=block" always;\nadd_header Referrer-Policy "strict-origin-when-cross-origin" always;\nadd_header Permissions-Policy "camera=(), microphone=(), geolocation=()" always;\nadd_header Content-Security-Policy "default-src \'self\'; script-src \'self\'; style-src \'self\' \'unsafe-inline\'" always;',
                "code_language": "nginx",
            },
            {
                "order": 2,
                "title": "Enable HSTS",
                "description": "Add the Strict-Transport-Security header to enforce HTTPS.",
                "code_snippet": 'add_header Strict-Transport-Security "max-age=31536000; includeSubDomains" always;',
                "code_language": "nginx",
            },
            {
                "order": 3,
                "title": "Verify headers are present",
                "description": "Check that all security headers are being sent in responses.",
                "code_snippet": "curl -sI https://yourdomain.com | grep -iE '(x-content|x-frame|strict|referrer|permissions|content-security)'",
                "code_language": "bash",
            },
        ],
    ...
    "sqli": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Eradicate Raw String Concatenation",
                "description": "Immediately refactor any codebase using string interpolation (e.g., Python f-strings) or direct concatenation for database queries.",
                "code_snippet": None,
                "code_language": None,
            },
            {
                ...
                "description": "Ensure the database driver treats user input strictly as literal values by utilizing parameterized queries (e.g., $1 variables in Postgres) or native ORM features.",
                "code_snippet": '/* Node.js (pg) Example */\nconst query = "SELECT * FROM accounts WHERE id = $1";\nconst result = await client.query(query, [userId]);',
                "code_language": "javascript",
            },
            {
                "order": 3,
                "title": "Deploy WAF Protection",
                "description": "Deploy Web Application Firewall rules utilizing complex regular expressions (e.g., `(?i)(\\b(UNION|SELECT|INSERT|UPDATE|DELETE|DROP)\\b)`) to intercept malicious payloads at the edge.",
                "code_snippet": None,
                "code_language": None,
            },
        ],
    ...
    "xss": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Enforce Context-Aware Output Encoding",
                "description": "Eliminate framework escape hatches (e.g., React's `dangerouslySetInnerHTML`, Laravel's `{!! !!}`, Django's `|safe` filter). Force all variables through default auto-escaping rendering engines.",
                "code_snippet": '<!-- Django Secure Example -->\n<div class="bio">{{ user.biography }}</div>',
                "code_language": "html",
            },
            {
                "order": 2,
                "title": "Sanitize Required Rich Text",
                "description": "If users must submit HTML (e.g., WYSIWYG editors), meticulously strip dangerous tags and event handlers (onerror, onload) using an industry-standard library like DOMPurify prior to database storage.",
                "code_snippet": "const cleanHTML = DOMPurify.sanitize(dirtyInput);",
                "code_language": "javascript",
            },
            {
                "order": 3,
                "title": "Deploy Content Security Policy (CSP)",
                "description": "Implement strict CSP headers to explicitly block the execution of unauthorized inline scripts.",
                "code_snippet": "Content-Security-Policy: default-src 'self'; script-src 'self'",
                "code_language": "http",
            },
            {
                "order": 4,
                "title": "Set HttpOnly Cookie Flags",
                "description": "Prevent JavaScript access to session tokens to mitigate the operational impact of a successful XSS payload.",
                "code_snippet": "Set-Cookie: session=xyz; HttpOnly; Secure; SameSite=Strict",
                "code_language": "http",
            },
        ],
    ...
    "csrf": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Configure SameSite Cookie Attributes",
                "description": "Instruct the client browser to never transmit session cookies during cross-site requests by setting the SameSite attribute.",
                "code_snippet": "Set-Cookie: session=abc; Secure; HttpOnly; SameSite=Lax",
                "code_language": "http",
            },
            {
                "order": 2,
                "title": "Implement Anti-CSRF Tokens",
                "description": "Require an unpredictable, server-generated validation token for all state-changing endpoints (POST, PUT, DELETE). Ensure middleware (e.g., csurf in Node, CsrfViewMiddleware in Django) is universally applied.",
                "code_snippet": '<!-- Laravel Example -->\n<form method="POST" action="/update">\n    @csrf\n    <button type="submit">Save</button>\n</form>',
                "code_language": "html",
            },
            {
                "order": 3,
                "title": "Enforce Custom Headers for SPAs",
                "description": "For API-driven Single Page Applications relying on CORS, mandate a custom HTTP header (e.g., `X-Requested-With: XMLHttpRequest`) which standard browsers block on cross-origin requests without explicit permission.",
                "code_snippet": None,
                "code_language": None,
            },
        ],
    ...
    "idor": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Implement Indirect Object References",
                "description": "Replace predictable, sequential auto-incrementing integers in client-facing APIs with cryptographically secure, unguessable identifiers (e.g., UUIDv4).",
                "code_snippet": None,
                "code_language": None,
            },
            {
                ...
                "description": "Ensure every data retrieval and manipulation function validates the requester's authenticated session against the target object's ownership metadata.",
                "code_snippet": "/* Express.js Secure Example */\nif (req.user.id !== requestedObject.ownerId && req.user.role !== 'ADMIN') {\n    return res.status(403).json({ error: \"Unauthorized\" });\n}",
                "code_language": "javascript",
            },
            {
                "order": 3,
                "title": "Utilize Framework Access Controls",
                "description": "Leverage centralized authorization frameworks such as Spring Security @PreAuthorize annotations, Laravel Policies, or Django object-level permissions to ensure rules are consistently applied.",
                "code_snippet": '/* Spring Boot Secure Example */\n@PreAuthorize("@securityService.isOwner(authentication, #id)")\npublic Order getOrder(@PathVariable Long id) { ... }',
                "code_language": "java",
            },
        ],
    ...
    "path_traversal": {
        ...
        "steps": [
            {
                "order": 1,
                "title": "Implement Indirect File Mapping",
                "description": "Never reference local filesystem paths directly via user input. Store a cryptographic hash or UUID in the database that acts as a secure, non-traversable proxy linking to the actual file path.",
                "code_snippet": None,
                "code_language": None,
            },
            {
                "order": 2,
                "title": "Strip Traversal Sequences",
                "description": "If dynamic file handling is strictly unavoidable, utilize language-specific functions to aggressively discard directory traversal characters (e.g., ../ or ..\\).",
                "code_snippet": "/* PHP Secure Example */\n$filename = basename($_GET['file']);",
                "code_language": "php",
            },
            {
                "order": 3,
                "title": "Enforce Canonicalization and Boundary Checks",
                "description": "Resolve the user input into a canonical absolute path (resolving symbolic links and relative sequences), then strictly verify the resulting path begins explicitly with the designated upload/storage root directory.",
                "code_snippet": "/* Node.js Secure Example */\nconst resolvedPath = path.resolve(rootDir, filename);\nif (!resolvedPath.startsWith(rootDir + path.sep)) {\n    throw new Error('Path traversal attempt');\n}",
                "code_language": "javascript",
            },
        ],
...
DEFAULT_FIX: dict = {
    ...
    "steps": [
        {
            "order": 1,
            "title": "Review the finding details",
            "description": "Examine the technical detail provided in the scan report to understand the exact nature of this vulnerability.",
            "code_snippet": None,
            "code_language": None,
        },
        {
            "order": 2,
            "title": "Research the remediation",
            "description": "Look up the specific vulnerability in the OWASP Testing Guide or CWE database for detailed remediation guidance.",
            "code_snippet": None,
            "code_language": None,
        },
        {
            "order": 3,
            "title": "Apply the fix",
            "description": "Implement the recommended configuration change or code fix based on the finding's fix_action field.",
            "code_snippet": None,
            "code_language": None,
        },
        {
            "order": 4,
            "title": "Verify and re-scan",
            "description": "After applying the fix, run another scan to confirm the vulnerability is resolved.",
            "code_snippet": None,
            "code_language": None,
        },
    ],
...
def generate_rule_based_fix(req: FixRequest) -> dict:
    ...
    base = (
        FIX_KNOWLEDGE.get(matched_cat, DEFAULT_FIX).copy()
        if matched_cat
        else DEFAULT_FIX.copy()
    )
    ...
    return {
        ...
        "impact": base["impact"],
        "steps": base["steps"],
        "verification": base.get(
            "verification", "Re-scan the target to confirm this finding is resolved."
        ),
