...
def process_dead_letter(scan_id: str, url: str, error_msg: str) -> None:
    ...
    """
    logger.error(f"[DLQ] Processing dead letter for scan {scan_id} ({url}): {error_msg}")
    try:
...
@celery.task(bind=True, name="run_scan", max_retries=3, autoretry_for=(Exception,), retry_backoff=True, retry_backoff_max=600, retry_jitter=True)
def run_scan(self, scan_id: str, url: str) -> None:
    ...
    """
    logger.info(f"Starting expanded scan task for scan_id: {scan_id}, url: {url} (Attempt {self.request.retries + 1}/{self.max_retries + 1})")
    ...
    try:
        ...
        if self.request.retries >= self.max_retries:
            ...
            process_dead_letter.apply_async(args=[scan_id, url, str(e)], queue="celery_dlq")
        raise e
