...
async def create_payment(
    request: Request, body: PaymentCreateRequest, db: AsyncSession = Depends(get_db)
) -> PaymentCreateResponse:
    ...
    if not scan or scan.status != "complete":
        raise HTTPException(
            status_code=400, detail="Scan not ready for payment or not found"
        )
    ...
    try:
        ...
        logger.error(
            f"Database error storing payment for scan_id={body.scan_id}: {e}",
            exc_info=True,
        )
        await db.rollback()
        raise HTTPException(
...
async def verify_payment(
    body: PaymentVerifyRequest, db: AsyncSession = Depends(get_db)
) -> PaymentVerifyResponse:
    ...
    report_result = await db.execute(
        select(Report).where(Report.scan_id == payment.scan_id)
    )
