...
async def create_share_link(
    ...
    if scan.user_id != current_user.id and current_user.role != "admin":
        raise HTTPException(
            status_code=403, detail="Only the report owner can create share links"
        )
...
async def list_share_links(
    ...
    result = await db.execute(
        select(ReportShareLink).where(
            ReportShareLink.scan_id == scan_id, not ReportShareLink.is_revoked
        )
    )
...
async def revoke_share_link(
    ...
    result = await db.execute(
        select(ReportShareLink).where(ReportShareLink.id == share_id)
    )
