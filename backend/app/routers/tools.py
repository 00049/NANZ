    ...
    try:
        ...
        for d in raw_result.header_details:
            ...
            headers_list.append({
                ...
                "fix": f"Configure {d['name']} correctly on your web server." if d["status"] != "pass" else ""
            })
...
async def check_email(request: Request, domain: str = Query(..., min_length=3)) -> dict:
    ...
    try:
        ...
        score = 100
        if not dns_dict.get("has_spf"): score -= 20
        elif dns_dict.get("spf_all_mechanism") == "+all": score -= 30
        if not dns_dict.get("has_dmarc"): score -= 20
        elif dns_dict.get("dmarc_not_enforced"): score -= 10
        if not dns_dict.get("has_dkim"): score -= 10
        if not dns_dict.get("has_mx"): score -= 10
        if not dns_dict.get("has_bimi"): score -= 5
        ...
        records.append({
            ...
            "risk": "Critical for preventing email spoofing." if not dns_dict.get("has_spf") else "SPF mechanism is weak." if dns_dict.get("spf_all_mechanism") == "+all" else "Good.",
            "fix": "Add a TXT record for SPF." if not dns_dict.get("has_spf") else "Change +all to ~all or -all." if dns_dict.get("spf_all_mechanism") == "+all" else ""
        })
        ...
        records.append({
            ...
            "risk": "Essential for email authentication alignment." if not dns_dict.get("has_dmarc") else "Policy is not enforced (p=none)." if dns_dict.get("dmarc_not_enforced") else "Good.",
            "fix": "Add a _dmarc TXT record." if not dns_dict.get("has_dmarc") else "Enforce policy to quarantine or reject." if dns_dict.get("dmarc_not_enforced") else ""
        })
        ...
        records.append({
            "type": "DKIM",
            "value": f"Found selectors: {', '.join(dns_dict.get('dkim_selectors_found', []))}" if dkim_found else "Missing",
            "status": "present" if dkim_found else "missing",
            "risk": "Cryptographic proof of email origin." if not dkim_found else "Good.",
            "fix": "Configure DKIM signing in your email provider." if not dkim_found else ""
        })
        
        # MX
        mx_found = dns_dict.get("has_mx", False)
        mx_records = dns_dict.get("mx_records", [])
        mx_value = ", ".join([m["server"] for m in mx_records]) if mx_records else "Missing"
        records.append({
            "type": "MX",
            "value": mx_value,
            "status": "present" if mx_found else "missing",
            "risk": "Required to receive emails." if not mx_found else "Good.",
            "fix": "Add MX records pointing to your mail server." if not mx_found else ""
        })
        ...
        records.append({
            ...
            "risk": "Improves brand visibility in inboxes." if not bimi_found else "Good.",
            "fix": "Publish a BIMI record with your verified logo." if not bimi_found else ""
        })
