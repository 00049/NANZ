...
async def invite_member(
    ...
    if not membership or membership.role not in ["owner", "admin"]:
        raise HTTPException(
            status_code=403, detail="Insufficient permissions to invite members"
        )
    ...
    if not target_user:
        raise HTTPException(
            status_code=404,
            detail="User not found with this email. They must register first.",
        )
