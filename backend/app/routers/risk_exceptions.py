...
from app.crud.crud_risk_exception import create_exception, get_active_exceptions_for_domain, remove_exception, get_exception
...
def add_exception(
    ...
    latest_report = db.execute(
        select(Report).join(Report.scan).where(Report.scan.has(domain_id=domain_id)).order_by(Report.generated_at.desc()).limit(1)
    ).scalars().first()
    ...
    if latest_report:
        ...
        # Update the report
        latest_report.risk_items = classified # Risk items were mutated in-place by _calculate_weighted_score
